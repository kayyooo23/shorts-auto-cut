...
class VideoStatus(str, enum.Enum):
    ...
    # Раньше означало "идёт поиск моментов через ИИ" — с тех пор, как поиск
    # моментов стал отдельным действием по требованию (POST /videos/{id}/
    # find-moments), а не автоматическим шагом сразу после транскрипции,
    # этот статус больше нигде не выставляется. Оставлен в enum ради
    # обратной совместимости со старыми строками в БД, если такие есть.
    FINDING_MOMENTS = "finding_moments"
    # Видео транскрибировано, редактор полностью открыт и рабочий — вне
    # зависимости от того, найдены ли уже моменты (ИИ или вручную) вообще.
    READY = "ready"
    FAILED = "failed"
